...
export const projectsData: Project[] = [
  ...
  {
    id: "nainistore",
    title: "NainiStore — Hyperlocal Marketplace",
    description:
      "End-to-end hyperlocal marketplace (Food, Bazaar, Electronics) with Spring Boot API, multi-store orders, zone-based delivery, seller/admin/delivery apps, Razorpay + COD settlements, and Firebase push notifications.",
    techStack: [
      "Spring Boot",
      "MongoDB",
      "React",
    ...
    features: [
      ...
      "Multi-pickup delivery UI — per-restaurant pickup stops, route hints, COD collection at door",
      "Zone-based delivery fees — polygon zones, ETA labels, out-of-zone checks",
      "Seller onboarding — KYC uploads, store branding, product CRUD, order management",
      "Admin panel — zones, sellers, products, withdrawals, CMS banners, COD settlements",
      "Payments — Razorpay online pay, COD with partner cash collection and platform settlement",
      "Auth — Email OTP, Google OAuth, JWT access/refresh tokens (~59-day sessions)",
      "Push notifications — Firebase messaging for customers, sellers, delivery partners",
    ],
    technologies: [
      "Java 21",
      "Spring Boot 3",
    ...
    keyFeatures: [
      "Layered Spring Boot architecture — catalog, orders, delivery, settlement, admin, auth modules",
      "ensureMasterOrderDeliveryGroup() — heals multi-store trips so all sub-orders share one delivery partner",
      "Deduped assignment list API with enriched pickupStops for mobile delivery app",
      "CatalogService with category-scoped search and store-name matching on product search",
      "CodSettlementService — COD cash collection, Razorpay pay-to-platform, seller payout release",
      "SPA rewrites on Vercel for deep links (order-tracking, store routes)",
    ],
    entities: [
      "Users",
      "Sellers",
    ...
    apiEndpoints: [
      ...
      "GET /api/delivery/assignments, PATCH /api/delivery/assignments/{id}/status — Delivery partner",
      "GET /api/delivery/cod-collections/{masterOrderId}/breakdown — COD settlement",
      "GET/POST /api/admin/zones, /sellers, /products — Admin",
    ...
    adminCredentials: {
      ...
      password: "Admin@123",
      note: "Default bootstrap admin after fresh DB; change in production.",
      loginUrl: "https://admin.nainistore.in",
    },
    skills: [
      "Spring Boot",
      "Spring Security",
